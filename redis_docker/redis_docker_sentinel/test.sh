MASERT_IP=$(docker inspect --format '{{ .NetworkSettings.IPAddress }}' rs_master_1)
SLAVE_IP=$(docker inspect --format '{{ .NetworkSettings.IPAddress }}' rs_slave_1)
SENTINEL_IP=$(docker inspect --format '{{ .NetworkSettings.IPAddress }}' rs_sentinel_1)
...
echo ------------------------------------------------
docker exec rs_sentinel_1 redis-cli -p 26379 info Sentinel
